          ...
          <div class="col-12">
            <div class="card">
              <div class="card-content">
                <div class="card-body">
                  <form multiple="form/data" @submit.prevent="updateProduct(product.id)">
                    <div class="row">
  ...
  methods: {
    ...
    getProductDetails() {
      axios.get(`/admin/product/${this.product_id}`)
          .then((response) => {
            ...
            this.product.blCategory            = response.data.product_bl_category_id;
            this.product.range                 = response.data.product_range_id;
            this.product.originalPrice         = response.data.original_price;
            this.product.type                  = response.data.product_type_id;
            this.product.noOfIdenticalProducts = response.data.no_of_identical_products;
            this.product.upcCode               = response.data.upc_code;
            this.product.noOfIdenticalProducts = response.data.no_of_identical_products;
            this.product.howToUse              = response.data.how_to_use;
            this.product.whoCanUse             = response.data.who_can_use;
            this.product.whatsItFor            = response.data.whats_it_for;
            if (response.data.images != null) {
              this.$refs.ImageUpload.Imgs = [];
              response.data.images.map(image => {
                this.$refs.ImageUpload.Imgs.push(`/storage/images/${image.image}`)
              })
    ...
    fileAppend() {
     ...
     // file.append('sku', sku);
      ...
      if (this.product.hasOwnProperty('image')) {
    ...
    async updateProduct(id) {
      ...
      axios.post(`/admin/product/${id}`, file, config)
          .then(() => {
            /*  this.getProductDetails();*/
            this.$toast.open({
